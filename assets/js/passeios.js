...
class PasseiosManager {
    ...
    return `
      <article class="tour-card">
        ...
        <div class="tour-content">
          ...
          <div class="tour-buttons icon-color-white">
  ...
  preencherDetalhes(passeio) {
    ...
    if (incluiLista && passeio.inclui) {
      incluiLista.innerHTML = passeio.inclui
        .map(item => `<li>${item}</li>`)
        .join('');
      // Processar emojis após inserir lista
    ...
    // Substituir emojis por ícones SVG após inserir conteúdo dinâmico
    // Chamar múltiplas vezes para garantir que todos os emojis sejam processados
    if (typeof window.replaceEmojisWithIconsGlobal === 'function') {
      setTimeout(() => {
        window.replaceEmojisWithIconsGlobal();
        // Usar função forçada também
        if (typeof window.forceReplaceEmojis === 'function') {
          window.forceReplaceEmojis();
        }
        // Segunda chamada após um delay maior para pegar conteúdo inserido posteriormente
        setTimeout(() => {
          window.replaceEmojisWithIconsGlobal();
          if (typeof window.forceReplaceEmojis === 'function') {
            window.forceReplaceEmojis();
  ...
  async renderizarGaleria(galeria) {
    ...
    // Substituir emojis por ícones SVG após renderizar galeria
    if (typeof window.replaceEmojisWithIconsGlobal === 'function') {
      setTimeout(() => {
        window.replaceEmojisWithIconsGlobal();
      }, 200);
